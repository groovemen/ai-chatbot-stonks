...
export default function ChatCard() {
  const [currentExchange, setCurrentExchange] = React.useState<Exchange | null>(null);
  const [currentStock, setCurrentStock] = React.useState<Stock | null>(null);
        ...
        <div className="flex gap-4">
          ...
          <div className="px-4 py-2 border border-white/40 rounded-lg mb-2 max-w-sm">
            ...
            <div className="flex flex-col">
              {stockData.map((exchange: { code: string; stockExchange: string; }, index: number) => (
                <Button className="mb-1" radius="none" key={exchange.code || index} disabled>
        ...
        <div className="flex gap-4">
          ...
          <div className="px-4 py-2 border border-white/40 rounded-lg mb-2 max-w-sm">
            <div className="flex flex-col">
              ...
              {currentExchange?.topStocks.map((stock: Stock, index: number) => (
                <Button className="mb-1" radius="none" key={stock.code || index} disabled>
                  {stock.stockName}
                </Button>
              ))}
            </div>
        ...
        {!currentExchange && (
          <div className="flex gap-4">
            ...
            <div className="px-4 py-2 border border-white/40 rounded-lg mb-2 max-w-sm">
              ...
              <div className="flex flex-col">
                {stockData.map((exchange: { code: string; stockExchange: string; }, index: number) => (
                  <Button className="mb-1" radius="none" key={exchange.code || index} onClick={() => handleSelectExchange(exchange)}>
                    {exchange.stockExchange}
                  </Button>
                ))}
              </div>
          ...
          <div className="flex gap-4">
            ...
            <div className="px-4 py-2 border border-white/40 rounded-lg mb-2 max-w-sm">
              <div className="flex flex-col">
                <h5 className="mb-4">Please select a Stock</h5>
                {currentExchange.topStocks.map((stock: Stock, index: number) => (
                  <Button className="mb-1" radius="none" key={stock.code || index} onClick={() => handleSelectStock(stock)}>
                    {stock.stockName}
                  </Button>
                ))}
              </div>
